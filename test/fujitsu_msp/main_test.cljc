(ns fujitsu-msp.main-test
  "Contract + behavioral test for the fujitsu-msp L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "fujitsu_msp-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'fujitsu-msp.main-test)]
