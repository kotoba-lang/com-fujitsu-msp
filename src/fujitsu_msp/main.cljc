(ns fujitsu-msp.main
  "Kotodama WASM entrypoint for the Fujitsu_msp clean-room actor (L4) — Clojure port.
  ...
  facts (`fujitsu_msp.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "fujitsu_msp")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"fujitsu_msp.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "fujitsu_msp-compat" :tier tier :entities entities} 200])
...
(defn start! [] :fujitsu_msp-compat/ready)
